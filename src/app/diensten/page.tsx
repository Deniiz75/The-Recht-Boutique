...
export default function DienstenPage() {
  return (
    <>
      ...
      <section aria-label="Overzicht van praktijkgebieden" className="bg-paper">
        <Container>
          <div className="py-16 lg:py-24">
            <ul className="grid gap-x-14 gap-y-16 lg:grid-cols-2">
              {services.map((service, index) => (
                <li key={service.slug} className="reveal">
                  <article className="group relative flex h-full flex-col border-t border-ink/25 pt-10">
                    <span
                      aria-hidden="true"
                      className="absolute -top-2.5 left-0 bg-paper pr-3 font-display text-lg leading-none text-rust-deep tabular-nums"
                    >
                    ...
                    <h2 className="mt-4 font-display text-heading">
                      <Link
                        ...
                        className="transition-colors after:absolute after:inset-0 after:content-[''] hover:text-rust-deep"
                      >
                        {service.title}
                      </Link>
                    </h2>
                    ...
                    <ul className="mt-7 space-y-2.5 border-t border-line pt-6">
                      {service.topics.slice(0, 3).map((topic) => (
                        ...
                        >
                          <span
                            aria-hidden="true"
                            className="absolute top-[0.7em] left-0 h-px w-2.5 bg-rust"
                          />
                          {topic}
                        </li>
                      ))}
                    ...
                    <span className="mt-8 inline-flex min-h-11 items-center gap-2 font-mono text-[0.625rem] tracking-[0.2em] text-ink uppercase">
                      ...
                      <ArrowUpRight
                        aria-hidden="true"
                        className="h-3.5 w-3.5 text-rust-deep transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5"
                      />
                    </span>
                  </article>
                </li>
      ...
      <ProcessSection surface="cream" withLink />
