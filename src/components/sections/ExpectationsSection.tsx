...
export function ExpectationsSection({ index }: ExpectationsSectionProps) {
  return (
    <section aria-labelledby="verwachten-titel" className="border-t border-line bg-paper">
      <Container>
        <div className="py-20 lg:py-28">
          ...
          <ul className="mt-16 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {expectations.map((item, itemIndex) => (
              <li
                key={item.title}
                className="reveal flex h-full flex-col border border-line bg-cream px-6 pt-9 pb-8"
                style={{ borderRadius: '96px 96px 0 0' }}
              >
                <span
                  aria-hidden="true"
                  className="mx-auto font-mono text-[0.625rem] tracking-[0.24em] text-rust-deep tabular-nums"
                >
                  {String(itemIndex + 1).padStart(2, '0')}
                </span>
                <span
                  aria-hidden="true"
                  className="mx-auto mt-4 block h-px w-8 bg-line"
                />
