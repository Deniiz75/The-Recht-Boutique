  ...
  return (
    <div className="lg:hidden">
      <button
        ...
        className="inline-flex h-11 min-w-11 items-center gap-2 border border-ink px-3 font-mono text-[0.6875rem] tracking-[0.18em] text-ink uppercase transition-colors hover:bg-ink hover:text-paper"
      >
        {open ? (
      ...
      <div
        ...
        className="on-dark fixed inset-0 z-100 flex flex-col overflow-y-auto bg-ink-deep px-5 pt-6 pb-12 text-paper"
      >
        <div className="flex items-center justify-between">
          <span className="font-mono text-[0.625rem] tracking-[0.28em] text-gold uppercase">
            Menu
          </span>
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="inline-flex h-11 min-w-11 items-center gap-2 border border-paper/40 px-3 font-mono text-[0.6875rem] tracking-[0.18em] text-paper uppercase transition-colors hover:bg-paper hover:text-ink-deep"
          >
            <X aria-hidden="true" className="h-4 w-4" />
            Sluiten
          </button>
          ...
          <ul className="border-t border-paper/15">
            {nav.map((item, index) => (
              <li key={item.href} className="border-b border-paper/15">
                <Link
                  href={item.href}
                  className="flex items-baseline gap-4 py-5 font-display text-[1.75rem] leading-tight text-paper transition-colors hover:text-gold"
                >
                  <span
                    aria-hidden="true"
                    className="font-mono text-[0.625rem] tracking-[0.2em] text-gold/70 tabular-nums"
                  >
            ...
            <li className="border-b border-paper/15">
              <Link
                href="/contact"
                className="flex items-baseline gap-4 py-5 font-display text-[1.75rem] leading-tight text-gold"
              >
                <span
                  aria-hidden="true"
                  className="font-mono text-[0.625rem] tracking-[0.2em] text-gold/70 tabular-nums"
        ...
        <div className="mt-auto pt-12">
          <p className="font-mono text-[0.625rem] tracking-[0.28em] text-gold uppercase">
            Direct contact
          </p>
          <div className="mt-4 flex flex-col gap-2 text-paper/90">
            {phone ? (
